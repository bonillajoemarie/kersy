...
- **Theme (owner, 2026-08-15): JetBrains Darcula, single dark theme — no light mode.** Exact palette as CSS tokens, taken from Darcula: background `#2B2B2B`, panel `#3C3F41`, borders `#323232`, default text `#A9B7C6`, selection `#214283`, caret line `#323232`. Code/accent colors follow Darcula's editor colors and are used semantically across the app: keyword-orange `#CC7832` (active/running), string-green `#6A8759` (success/verified), number-blue `#6897BB` (info/links/sessions), function-yellow `#FFC66D` (warnings/context gauge amber), comment-gray `#808080` (stale/disabled), error-red `#BC3F3C` (failures/zero-verification badge). Shell commands and file paths render in a monospace font with these token colors — the drill-in feed should read like a JetBrains editor.
- **GPU-accelerated rendering (owner, 2026-08-15):** the graph renders through **WebGL2** — nodes as instanced quads/point sprites, edges as line primitives, with two tiny hand-written shaders (no rendering library; well under the dependency budget). Labels and the pulse glow render on a Canvas2D overlay (text is where WebGL is weak). Physics stays in d3-force on the CPU (it's cheap); only drawing moves to the GPU. Fallback: if WebGL2 context creation fails, the same draw list renders via Canvas2D — one renderer interface (`draw(nodes, edges, camera)`), two backends. Target: 60 fps pans/zooms with every session on the machine on screen, and still 0% GPU/CPU when the simulation settles (frame loop fully stops).
